...
PROJECT_ROOT = os.path.abspath(os.path.join(VERSION_ROOT, "..", ".."))

app = Flask(
    __name__,
    template_folder=os.path.join(VERSION_ROOT, "templates"),
...
app.config['SESSION_COOKIE_NAME'] = 'banko_v2_session'

# Add abs filter to Jinja2
app.jinja_env.filters['abs'] = abs

# KYC file upload configuration
UPLOAD_FOLDER = os.path.join(VERSION_ROOT, 'uploads', 'kyc_documents')
...
DATABASE_URI = os.environ.get(
    'BANKO_V2_DATABASE_URI',
    'cockroachdb://root@localhost:26257/bank_v2?sslmode=disable',
)
...
AI = BankoAIServices(
    ...
    docs_dir=os.path.join(VERSION_ROOT, 'data', 'docs'),
    index_dir=os.path.join(VERSION_ROOT, 'data', 'chroma_index'),
)
...
def _vaba_current_user():
    ...
    try:
        ...
        username = rows[0][1] if rows else "unknown"
    except Exception:
        username = "unknown"
    return {
        "id": user_id,
...
@app.route('/vaba', methods=['GET'])
def vaba_dashboard():
    if 'user_id' not in session:
        return redirect(url_for('login'))
    scenarios = vaba_lab.list_scenarios()
    runs = vaba_lab.list_evidence_runs()[:8]
    return render_template(
        'vaba.html',
        scenarios=scenarios,
        runs=runs,
        user=_vaba_current_user(),
    )


@app.route('/vaba/<scenario_id>', methods=['GET'])
def vaba_scenario_detail(scenario_id):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    try:
        scenario = vaba_lab.get_scenario(scenario_id)
    except vaba_lab.VABALabError:
        return redirect(url_for('vaba_dashboard'))

    run_id = request.args.get('run_id')
    evidence = None
    if run_id:
        try:
            evidence = vaba_lab.read_evidence_run(run_id)
        except Exception:
            evidence = None

    # A scenario may declare which live labs demonstrate the same lesson for real. VABA
    # itself never calls a model: every model call in this app either goes through the
    # spooled broker/sandbox path every live lab uses, or is this in-process simulation.
    # Resolving to a link rather than a new in-process call keeps that boundary intact.
    related_labs = []
    for lab_id in scenario.get('related_live_labs') or []:
        try:
            lab = lab_runner.get_lab(lab_id)
            related_labs.append({'id': lab_id, 'title': lab['title'], 'tier': lab['tier']})
        except lab_runner.LabRunnerError:
            continue

    # Each PoC pairs with a specific tool/source value, not just prompt text: VABA01's
    # tool dropdown is passed as requested_tool and overrides whatever the prompt would
    # otherwise infer, and VABA03's source dropdown picks the URL from an entirely
    # different place (a fixed tool payload, a doc on disk, or the prompt) regardless of
    # what the textarea says. A picker that only filled the textarea would silently
    # demonstrate the wrong tool or fetch the wrong URL depending on whatever the
    # dropdown last happened to be set to. Computed here by reusing the same inference
    # the server itself uses, so this cannot drift from what actually runs.
    poc_metadata = []
    for poc in scenario.get('pocs') or []:
        entry = {'text': poc}
        if scenario_id in ('VABA01', 'VABA02'):
            entry['tool'] = vaba_lab.infer_requested_tool(poc)
        elif scenario_id == 'VABA03':
            low = poc.lower()
            if 'tool output says' in low:
                entry['source'] = 'tool_output'
            elif 'rag note says' in low:
                entry['source'] = 'rag'
            else:
                entry['source'] = 'prompt'
        poc_metadata.append(entry)
    ...
    return render_template(
        ...
        evidence=evidence,
        user=_vaba_current_user(),
        default_prompt=(scenario.get('pocs') or [''])[0],
        tools=vaba_lab.TOOL_REGISTRY,
        related_labs=related_labs,
        poc_metadata=poc_metadata,
    )


@app.route('/vaba/<scenario_id>/run', methods=['POST'])
def vaba_run_scenario(scenario_id):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    try:
        scenario = vaba_lab.get_scenario(scenario_id)
    ...
    if mode not in ('vulnerable', 'secure'):
        mode = 'vulnerable'
    user_input = request.form.get('prompt') or (scenario.get('pocs') or [''])[0]
    current_user = _vaba_current_user()
    requested_tool = request.form.get('tool') or None
    events = []

    if scenario_id == 'VABA01':
        events.append(vaba_lab.simulate_tool_decision(user_input, mode, current_user, requested_tool))
    elif scenario_id == 'VABA02':
        events.append(vaba_lab.simulate_orchestration_trace(user_input, mode, current_user, requested_tool))
        events.append(vaba_lab.simulate_tool_decision(user_input, mode, current_user, requested_tool))
    elif scenario_id == 'VABA03':
        source = request.form.get('source', 'prompt')
        if source not in ('prompt', 'tool_output', 'rag'):
            source = 'prompt'
        target = vaba_lab.resolve_fetch_target(user_input, source)
        ...
        events.append(
            vaba_lab.simulate_output_handling(
                vaba_lab.compose_model_answer(user_input, fetch_event),
                mode,
                source=source,
            )
        )
    ...
    return redirect(url_for('vaba_scenario_detail', scenario_id=scenario_id, run_id=evidence['run_id']))


@app.route('/vaba/reset', methods=['POST'])
def vaba_reset():
    if 'user_id' not in session:
        return redirect(url_for('login'))
    # Unified V2 reset: simulated evidence, live-lab evidence, spooled run specs,
    # and captured sandbox results. V1 paths and the `bank` database are untouched.
    removed = lab_evidence.reset_v2_lab_state()
    if request.headers.get('X-Requested-With') == 'fetch' or request.is_json:
        return jsonify({'removed': removed})
    return redirect(url_for('vaba_dashboard'))


@app.route('/vaba/evidence/<run_id>', methods=['GET'])
def vaba_evidence(run_id):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    try:
        evidence = vaba_lab.read_evidence_run(run_id)
        scenario = vaba_lab.get_scenario(evidence['scenario_id'])
    except Exception:
        return redirect(url_for('vaba_dashboard'))
    return render_template(
        'vaba_scenario.html',
        ...
        evidence=evidence,
        user=_vaba_current_user(),
        default_prompt=evidence.get('user_input') or (scenario.get('pocs') or [''])[0],
        tools=vaba_lab.TOOL_REGISTRY,
    )

@app.route('/catalog', methods=['GET'])
def unified_catalog():
    """One index across all three teaching surfaces.

    They stay separate pages because they behave differently — the LLM catalog and VABA
    are simulation-only and safe to demonstrate anywhere, while live labs cost money and
    execute real code. This page exists so a learner can see the whole surface and, more
    importantly, see which parts of it actually run.
    """
    if 'user_id' not in session:
        return redirect(url_for('login'))
    ...
    for lab_id, lab in lab_runner.LAB_REGISTRY.items():
        tier = lab_runner.get_tier(lab['tier'])
        live.append({
            'id': lab_id,
            'title': lab['title'],
            ...
            'credentials': tier['credentials_allowed'],
            'implemented': bool(lab.get('implemented')),
        })

    return render_template(
        'catalog.html',
        scenarios=scenarios,
        vulns=LLM_VULNERABILITIES,
        live=live,
        tiers=lab_runner.TIERS,
        budget={'spent': lab_budget.spent_usd(), 'cap': lab_budget.TOTAL_CAP_USD},
        user=_vaba_current_user(),
    )
...
def labs_dashboard():
    ...
    return render_template(
        'labs.html',
        labs=lab_runner.LAB_REGISTRY,
        tiers=lab_runner.TIERS,
        pending=lab_runner.list_specs(),
        runs=[r for r in lab_evidence.list_runs() if r.get('kind') == 'live'][:8],
        budget={
            'spent': lab_budget.spent_usd(),
            'remaining': lab_budget.remaining_usd(),
            'cap': lab_budget.TOTAL_CAP_USD,
            'per_run_cap': lab_budget.PER_RUN_CAP_USD,
        },
        user=_vaba_current_user(),
    )
...
def lab_detail(lab_id):
    ...
    try:
        ...
        return redirect(url_for('labs_dashboard'))

    # The run_id on this route is always the *spooled* run_id from build_run_spec, the
    # one the "check on my queued run" link carries forward. That is not the same ID
    # evidence ends up filed under: capture_evidence() in the broker generates a fresh
    # run_id for the evidence record itself and keeps the spooled one only as
    # broker_run_id inside it. lab_evidence.read_run() looks evidence up by its own ID
    # and would never find anything here, queued or not -- found by testing this fix
    # live, not by inspection. lab_runner.read_result() is keyed by the spooled ID
    # specifically for this lookup and already existed, unused, for exactly this.
    run_id = request.args.get('run_id')
    evidence = None
    if run_id:
        try:
            evidence = lab_runner.read_result(run_id)
        except lab_runner.LabRunnerError:
            evidence = None
    ...
    return render_template(
        ...
        lab=lab,
        tier=lab_runner.get_tier(lab['tier']),
        default_inputs=json.dumps(lab.get('default_inputs') or {}, indent=2),
        presets=lab_llm.MODEL_PRESETS,
        default_preset=lab_llm.DEFAULT_PRESET,
        evidence=evidence,
...
@app.route('/labs/<lab_id>/run', methods=['POST'])
def lab_run(lab_id):
    """Spool a run spec. The app never executes a lab payload; the broker does."""
    if 'user_id' not in session:
        return redirect(url_for('login'))
    try:
        lab_runner.get_lab(lab_id)
    except lab_runner.LabRunnerError:
        return redirect(url_for('labs_dashboard'))
    ...
    if mode not in ('vulnerable', 'secure'):
        mode = 'vulnerable'
    try:
        inputs = json.loads(request.form.get('inputs') or '{}')
        if not isinstance(inputs, dict):
            raise ValueError('inputs must be a JSON object')
    except Exception:
        return redirect(url_for('lab_detail', lab_id=lab_id, queued='invalid_inputs'))
    ...
    try:
        ...
        lab_runner.submit_spec(spec)
    except lab_runner.LabRunnerError:
        return redirect(url_for('lab_detail', lab_id=lab_id, queued='refused'))

    # Carry run_id forward too, not just queued: without it, the very next page load
    # never even attempts to look up evidence, so re-checking meant reconstructing the
    # URL by hand. With it, this same URL keeps working once the broker has run.
    return redirect(url_for('lab_detail', lab_id=lab_id, queued=spec['run_id'], run_id=spec['run_id']))


@app.route('/labs/<lab_id>/compare', methods=['POST'])
def lab_compare_run(lab_id):
    """Queue the same attack once per model preset.

    The contrast between presets is the teaching device: identical input, identical
    mode, only the model tier differs. Queuing them as one group keeps that comparison
    honest, because the inputs cannot drift between runs.
    """
    if 'user_id' not in session:
        return redirect(url_for('login'))
    try:
    ...
    comparison_id = 'cmp-' + uuid.uuid4().hex[:10]
    queued = 0
    for preset_name in lab_llm.MODEL_PRESETS:
        try:
            spec = lab_runner.build_run_spec(
                lab_id, mode, user=_vaba_current_user(), inputs=inputs,
                preset=preset_name, comparison_id=comparison_id,
            )
        except lab_runner.LabRunnerError:
            # A preset this tier may not use is skipped rather than failing the group.
            continue
        lab_runner.submit_spec(spec)
        queued += 1
    ...
    if not queued:
        return redirect(url_for('lab_detail', lab_id=lab_id, queued='refused'))
    return redirect(url_for('lab_compare_view', comparison_id=comparison_id))


@app.route('/labs/compare/<comparison_id>', methods=['GET'])
def lab_compare_view(comparison_id):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    if not re.fullmatch(r'cmp-[a-f0-9]{10}', comparison_id or ''):
        return redirect(url_for('labs_dashboard'))

    runs = [r for r in lab_evidence.list_runs() if r.get('comparison_id') == comparison_id]
    pending = [s for s in lab_runner.list_specs() if s.get('comparison_id') == comparison_id]

    lab = None
    if runs:
        try:
            lab = lab_runner.get_lab(runs[0]['lab_id'])
        except lab_runner.LabRunnerError:
            lab = None
    elif pending:
        try:
            lab = lab_runner.get_lab(pending[0]['lab_id'])
        except lab_runner.LabRunnerError:
            lab = None

    rows = []
    for run in sorted(runs, key=lambda r: r.get('preset') or ''):
        event = (run.get('events') or [{}])[0]
        key = (lab or {}).get('headline_key')
        rows.append({
            'preset': run.get('preset'),
            'mode': run.get('mode'),
            'model': (event.get('model_call') or {}).get('model'),
            'simulated': (event.get('model_call') or {}).get('simulated'),
            'outcome': event.get('outcome'),
            'headline': event.get(key) if key else None,
...
@app.route('/labs/evidence/<run_id>', methods=['GET'])
def lab_evidence_view(run_id):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    try:
        evidence = lab_evidence.read_run(run_id)
        lab = lab_runner.get_lab(evidence['lab_id'])
    except Exception:
        return redirect(url_for('labs_dashboard'))
    return render_template(
        'lab_detail.html',
